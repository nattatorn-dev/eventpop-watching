    ...
    for (i = 0; i < document.getElementsByTagName('script').length; i++) {
      if (
        document
          ...
          .outerHTML.indexOf('window.gon') !== -1
      ) {
        selectScriptIndex = i
      }
    }

    var selectorWindowGon = document
      .querySelectorAll('script')[selectScriptIndex]
      .outerHTML.split(';gon.')
    var inject = selectorWindowGon.reduce(function(p, c, k) {
      return Object.assign(p, {
        [c.substr(0, c.indexOf('='))]: c.substr(c.indexOf('=') + 1, c.length)
      })
    }, {})
    var ticketAvailable = JSON.parse(inject.ticket_types_min)
    var localStorageKey =
      'events:' + inject.event_id + ':ticket_types:pending_orders'
    var localStorageValue = JSON.stringify(
      Object.keys(ticketAvailable).reduce(function(p, c, k) {
        return k === 0
          ? Object.assign(p, { [c]: 1 })
          : Object.assign(p, { [c]: ticketAvailable[c] })
      }, {})
    )
